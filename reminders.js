...
export function reminderItems(state,{asOf=todayLocal(),days=30,includeOverdue=true,settings=reminderSettings(state)}={}) {
  ...
  if (settings.includePayments) for (const item of occurrences(state,through,asOf,from)) if (!['paid','skipped'].includes(item.status)) items.push({id:`payment-${item.schedule}-${item.due}`,kind:'payment',source:item.schedule,due:item.effectiveDue,originalDue:item.due,title:item.name,description:item.type==='income'?'Erwarteter Eingang':'Geplante Zahlung',amount:item.amount});
...
export function calendarICS(items,{settings=reminderDefaults(),now=new Date()}={}) {
  ...
  const lines=['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//Hany//Erinnerungen 1.0.0//DE','CALSCALE:GREGORIAN','X-WR-CALNAME:Hany Erinnerungen'];
