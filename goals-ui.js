...
export function goalsUI(c) {
  ...
  function practiceDetail(date) {
    ...
    openSheet(`<h1 class="sheet-title">${label(date)}</h1>${p.goalId?`<p class="subtitle">${esc(goal(p.goalId)?.title||'')}</p>`:''}${p.action?`<div class="card"><h3>Mein Schritt</h3><p class="goal-prewrap">${esc(p.action)}</p></div>`:''}${p.note?`<div class="card"><h3>Meine Erfahrung</h3><p class="goal-prewrap">${esc(p.note)}</p></div>`:''}${p.confidence!==null?`<p>Meine Zuversicht: <strong>${p.confidence} / 5</strong></p>`:''}<h2>An diesem Tag gelesen</h2>${p.readIds.map(id=>`<blockquote class="card belief-history">„${esc(p.readTexts?.[id]||belief(id)?.text||'')}“</blockquote>`).join('')||'<p class="hint">Keine Sätze als gelesen markiert.</p>'}<p class="hint">Ab diesem Update wird die gelesene Formulierung bewahrt. Ältere Einträge zeigen die aktuell verfügbare Formulierung.</p><button class="primary" data-practice-edit="${date}">Eintrag bearbeiten</button>`);
  }
