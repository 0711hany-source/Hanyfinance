import {today, uid, cents,balance} from './model.js';
...
export function goalsUI(c) {
  ...
  const effectiveSaved=g=>g.linkedAccount?Math.max(0,balance(state(),g.linkedAccount)):goalSaved(g);
  const effectiveProgress=g=>goalProgress(g.linkedAccount?{...g,progress:[{amount:effectiveSaved(g)}]}:g);
  const effectivePlan=g=>goalPlan(g.linkedAccount?{...g,progress:[{amount:effectiveSaved(g)}]}:g,today());
  ...
  function goalRows(gs) {
    return gs.length?gs.map(g=>`<div class="row goal goal-rich ${g.done?'done':''}" data-goal="${g.id}"><button class="check ${g.done?'done':''}" data-toggle="${g.id}" aria-label="${g.done?'Wieder öffnen':'Als erreicht markieren'}: ${esc(g.title)}">${g.done?icon('check'):''}</button><button class="row-body goal-open" data-g-open="${g.id}"><strong>${g.pinned?'☆ ':''}${esc(g.title)}</strong><small>${g.horizon} · ${dueText(g)}</small>${g.kind==='savings'?`<span class="goal-mini-amount">${eur(effectiveSaved(g))} <span>von ${eur(g.targetCents)}</span></span>`:g.milestones.length?`<small>${g.milestones.filter(m=>m.done).length} von ${g.milestones.length} Etappen erledigt</small>`:''}${g.kind==='savings'||g.milestones.length?`<span class="goal-meter" role="progressbar" aria-label="Fortschritt ${esc(g.title)}" aria-valuenow="${effectiveProgress(g)}" aria-valuemin="0" aria-valuemax="100"><span style="width:${effectiveProgress(g)}%"></span></span>`:''}${g.nextAction&&!g.done?`<span class="goal-next">Als Nächstes: ${esc(g.nextAction)}</span>`:''}</button><button class="tiny goal-chevron" data-g-open="${g.id}" aria-label="Details zu ${esc(g.title)}">›</button></div>`).join(''):'<p class="empty">Hier ist Platz für deinen nächsten Schritt.<br>Lege ein Ziel an oder passe eine Vorlage an.</p>';
  }
  function goals() {
  ...
  function toggleGoal(id) {
    ...
    if(!g.done&&g.kind==='savings'&&effectiveSaved(g)<g.targetCents){c.toast('Trage zuerst den erreichten Sparbetrag ein oder passe dein Ziel an.');return false;}
  ...
  function beliefCard(b, readable=false) {
    ...
    return `<article class="card belief-card"><p class="eyebrow">${g?esc(g.title):'DEINE AUSRICHTUNG'}</p><blockquote>„${esc(b.text)}“</blockquote>${b.nextAction?`<p class="hint">Nächster Schritt: ${esc(b.nextAction)}${b.trigger?' · '+esc(b.trigger):''}</p>`:''}${b.credibility!==null&&b.credibility!==undefined?`<span class="pill">Glaubwürdig ${b.credibility}/10</span>`:''}${b.oldThought||b.evidence?`<details><summary>Gedanke & Erfahrungen</summary>${b.oldThought?`<p><strong>Bisheriger Gedanke</strong><br>${esc(b.oldThought)}</p>`:''}${b.evidence?`<p><strong>Was für meinen neuen Satz spricht</strong><br>${esc(b.evidence)}</p>`:''}</details>`:''}<div class="belief-actions">${readable?`<button class="quick ${read?'secondary':''}" data-b-read="${b.id}" ${read?'disabled':''}>${read?'Heute gelesen ✓':'Bewusst gelesen'}</button>`:''}<button class="tiny" data-b-edit="${b.id}">Bearbeiten</button><button class="tiny" data-thought-review="${b.id}">Erfahrung & Verlauf</button></div></article>`;
  }
  function focus() {
  ...
  function dailyView() {
    const practice=state().practice.find(p=>p.date===today()),beliefs=activeBeliefs().slice(0,3),read=beliefs.filter(b=>practice?.readIds.includes(b.id)).length,targets=activeGoals().slice(0,3);
    return `<div class="goal-daily-head"><span class="pill">${new Date().toLocaleDateString('de-DE',{weekday:'long',day:'numeric',month:'long'})}</span><span>${read}/${beliefs.length} persönliche Sätze gelesen</span></div>${c.ritualCards?.()||''}${section('Deine Ziele im Blick')}<div class="card">${goalRows(targets)}${!targets.length?'<button class="tiny" data-action="new-goal">Mein erstes Ziel anlegen →</button>':''}</div>${section('Deine persönlichen Glaubenssätze','thought-assistant','Assistent')}<p class="hint">Der Tagesimpuls ist eine zusätzliche Anregung. Hier bleiben deine eigenen Sätze und Erfahrungen.</p>${beliefs.length?beliefs.map(b=>beliefCard(b,true)).join(''):'<button class="quick secondary wide" data-action="new-belief">Meinen ersten Glaubenssatz schreiben</button>'}<button class="quick secondary wide" data-action="goal-week-review">Diese Woche bewusst abschließen →</button>`;
  }
  ...
  function dailyForm(date=today()) {
    if(c.smartCheckin)return c.smartCheckin(date);
  ...
  function practiceDetail(date) {
    ...
    openSheet(`<h1 class="sheet-title">${label(date)}</h1>${p.goalId?`<p class="subtitle">${esc(goal(p.goalId)?.title||'')}</p>`:''}${p.action?`<div class="card"><h3>Mein Schritt</h3><p class="goal-prewrap">${esc(p.action)}</p></div>`:''}${p.note?`<div class="card"><h3>Meine Erfahrung</h3><p class="goal-prewrap">${esc(p.note)}</p></div>`:''}${p.confidence!==null?`<p>Meine Zuversicht: <strong>${p.confidence} / 5</strong></p>`:''}${c.ritualHistory?.(p)||''}<h2>An diesem Tag gelesen</h2>${p.readIds.map(id=>`<blockquote class="card belief-history">„${esc(p.readTexts?.[id]||belief(id)?.text||'')}“</blockquote>`).join('')||'<p class="hint">Keine Sätze als gelesen markiert.</p>'}<p class="hint">Ab diesem Update wird die gelesene Formulierung bewahrt. Ältere Einträge zeigen die aktuell verfügbare Formulierung.</p><div class="grid2"><button class="quick secondary" data-ritual="morning" data-date="${date}">Morgenroutine bearbeiten</button><button class="quick secondary" data-ritual="evening" data-date="${date}">Abendblick bearbeiten</button></div><button class="tiny" data-practice-edit="${date}">Allgemeinen Eintrag bearbeiten</button>`);
  }
  function scienceNote() {
    return `<details class="card goal-science"><summary>Wie diese Praxis gedacht ist</summary><p>Stelle dir dein gewünschtes Ergebnis vor. Benenne dann ein reales Hindernis und einen konkreten Wenn-dann-Plan. Diese Struktur orientiert sich an WOOP (Wunsch, Ergebnis, Hindernis, Plan).</p><p>Nutze Glaubenssätze als glaubwürdige Erinnerung an dein Handeln. Überprüfe sie anhand deiner Erfahrungen und passe sie an, wenn sie sich unpassend anfühlen.</p><p>Gedanken allein garantieren weder Geld noch einen bestimmten Erfolg. Joe Dispenza und Napoleon Hill sind keine Wirksamkeitsnachweise für Hany; Aussagen über das Anziehen von Ereignissen werden hier nicht als wissenschaftliche Tatsachen dargestellt.</p><button class="tiny wide" data-action="smart-science">Studien und Grenzen ansehen</button><p><a href="https://woopmylife.org/en/science" target="_blank" rel="noopener">WOOP: Forschung und Methode ↗</a></p></details>`;
  }
  ...
  const actions={
    ...
    'read-all-beliefs':()=>c.commit(()=>markBeliefsRead(state(),activeBeliefs().slice(0,3).map(b=>b.id),today()),'Dein Lesemoment ist festgehalten'),
